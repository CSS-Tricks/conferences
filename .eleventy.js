...
module.exports = function (config) {
  config.addCollection('events', function (collection) {
    ...
    let futureEvents = allEvents.filter(conf => {
  ...
  config.addCollection('pastevents', function (collection) {
    let allEvents = collection.getFilteredByGlob('site/events/*.md');
    let pastEvents = allEvents.filter(conf => {
      return conf.data.date <= new Date();
    });
    return pastEvents;
  });
  ...
  config.addFilter('getEventYears', (events) => {
    let result = events.reduce((years, conf) => {
  ...
  config.addFilter('doesConfExist', (events, monthToTest, yearToTest) => {
    let length = events.filter(conf => {
